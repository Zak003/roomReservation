...
function whileSQL($con, $day, $time){
  ...
  while($row=mysqli_fetch_assoc($result)){
    $c = false;
    $cas = new DateTime(date("Y-m-d",$day)." $time:30");
    $zacetek = new DateTime($row["start_time"]);
    $konec = new DateTime($row["end_time"]);
    if (($cas > $zacetek) && ($cas <= $konec)){
      if($row['room_id'] == 1)
      {
          echo "<div class='event'>".$row["room_name"]."<div class='overlay_event'>"."<span class='text_overlay'>"."Name of reservation: ".$row["reservation_name"]."</span>"."</div>"."</div>";
      }
      else if($row['room_id'] == 3)
      {
        echo "<div class='drugi_event'>".$row["room_name"]."<div class='overlay_event'>"."<span class='text_overlay'>"."Name of reservation: ".$row["reservation_name"]."</span>"."</div>"."</div>";
      }
      else if($row['room_id'] == 5)
      {
        echo "<div class='tretji_event'>".$row["room_name"]."<div class='overlay_event'>"."<span class='text_overlay'>"."Name of reservation: ".$row["reservation_name"]."</span>"."</div>"."</div>";
      }
      else
      {
        echo "<div class='cetrti_event'>".$row["room_name"]."<div class='overlay_event'>"."<span class='text_overlay'>"."Name of reservation: ".$row["reservation_name"]."</span>"."</div>"."</div>";
      }
    }
    else{
      //echo "<a class='dayInWeek' href='rezervacija.php?&day=$day$&timeRez=$time'>&nbsp;</a>";
...
<body>
    <div class="banner">
      ...
      <?php echo '<a href="rezervacija.php?ajdi_sobe='.$room_id.'" class="overlay_povezava">Book for '.$room_name.'</a>';?> 
      </div>
